...
__global__ void convolution2D_kernel(const float* input, const float* kernel, float* output,
...
void solve(const float* input, const float* kernel, float* output, int input_rows, int input_cols, int kernel_rows, int kernel_cols){
    ...
    size_t inputsize = input_rows * input_cols * sizeof(float);
    size_t kernelsize = kernel_rows * kernel_cols * sizeof(float);
    size_t outputsize = output_rows * output_cols * sizeof(float);
...
int main(){
    ...
    std::vector<float> input(input_rows * input_cols), kernel(kernel_rows * kernel_cols), output(output_rows * output_cols);
